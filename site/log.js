...
(function () {
  ...
  /* the Back key of a TV remote: Samsung sends 10009, LG 461, others a named key */
  function isBack(e) { return e.keyCode === 10009 || e.keyCode === 461 || e.key === 'GoBack' || e.key === 'BrowserBack' || (e.key === 'Backspace' && !(e.target.closest && e.target.closest('input,textarea,[contenteditable]'))); }
  ...
  function loadEdits() {
    return fetch(EDITS_URL, { cache: 'no-cache' }).then(function (r) { return r.ok ? r.json() : null; }).catch(function () { return null; })
      .then(function (e) { if (e && typeof e === 'object') EDITS = e; EDITS.days = EDITS.days || {}; EDITS.photos = EDITS.photos || {}; EDITS.sections = EDITS.sections || {}; applyPhotoEdits(); });
  }
    ...
    return fetch('/log/' + USER + '/whoami', { cache: 'no-store', credentials: 'same-origin' }).then(function (r) { return r.json().catch(function () { return {}; }); })
      .catch(function () { return {}; }).then(function (me) {
        ...
        document.querySelectorAll('.wrap > .main').forEach(function (main) {
          ...
          var ft = main.querySelector('.foot'); if (ft) main.insertBefore(p, ft); else main.appendChild(p);
        });
  ...
  function textOf(key, lang, kind) {
    if (typeof key === 'number') {
      if (kind !== 'tips') return dayText(key, lang);
      var t = EDITS.days[key] && EDITS.days[key].tips && EDITS.days[key].tips[lang]; if (t) return t.slice();
      var d = DAYS[key] && DAYS[key].tips; return d && d[lang] ? d[lang].slice() : [];
    }
    if (key === 'intro' || key === 'outro') { var e = EDITS[key] && EDITS[key][lang]; if (e) return e.slice(); var s = LOG[key] && LOG[key][lang]; return s ? s.slice() : []; }
    var es = EDITS.sections && EDITS.sections[key] && EDITS.sections[key][lang]; if (es) return es.slice();
    var ss = LOG.sections && LOG.sections[key] && LOG.sections[key][lang]; return ss ? ss.slice() : [];
  }
  ...
  function redrawLog(st, kind) {
    var n = +st.getAttribute('data-day'), lang = langOf(st), pane = st.querySelector('.pane[data-pane="' + (kind === 'tips' ? 'tips' : 'log') + '"]'); if (!pane) return;
    pane.innerHTML = linkPlaces(textOf(n, lang, kind).map(function (p) { return blocks(p, lang); }).join('\n'));
    if (kind === 'tips') st._editedTips = true; else st._edited = true;
  }
  function saveText(key, texts, lang, kind) {
    var body = typeof key === 'number' ? (kind === 'tips' ? { day: key, tips: texts } : { day: key, text: texts }) : { section: key, text: texts };
    return postEdit(body, lang).then(function () {
      if (typeof key === 'number') document.querySelectorAll('.stage[data-day="' + key + '"]').forEach(function (st) { if (kind === 'tips') st._editedTips = false; else st._edited = false; redrawLog(st, kind); });
  ...
  function openEditor(st, at, kind) {
    if (!st) return;
    kind = kind === 'tips' ? 'tips' : 'log';
    var isDay = st.hasAttribute('data-day'), n = isDay ? +st.getAttribute('data-day') : st.getAttribute('data-section'), lang = langOf(st);
    var b = isDay ? st.querySelector('.tabs .editbtn') : st.querySelector(':scope > .editbtn'), pane = isDay ? st.querySelector('.pane[data-pane="' + kind + '"]') : st.querySelector('.sectext');
    if (!b || !pane) return;
  ...
  function editAt(p, x, y) {
    ...
    var kind = p.closest('.pane[data-pane="tips"]') ? 'tips' : 'log';
    var lang = langOf(st), paras = textOf(st.hasAttribute('data-day') ? +st.getAttribute('data-day') : st.getAttribute('data-section'), lang, kind);
    var node = null, off = 0;
    ...
    if (idx < 0) { openEditor(st, 0, kind); return; }
  ...
  /* ---- the map tracks the story: beside the text (body.spine-on) with the Track button on, the map glides to
     the picture nearest the reading line whenever that picture changes, and stays there between pictures; before
     a day's first picture it shows the day's stretch. Snapping, not scrolling: one flight per picture. ---- */
  var trackKey = null, trackTimer = null;
  function trackStops() {
  ...
  (function () {
    ...
    function blocks() {
      ...
      return Array.prototype.filter.call(w.querySelectorAll('h2[id], .stage[data-day], .pane p, .pane .photoref, .pane .photos, .sectext p, .sectext .photoref, .sectext .photos, .facts'), function (el) {
        return el.offsetParent !== null && !el.closest('.photos .photoref, .editor, .hiddenpics, .signin, .foot, .wx, .wxh') && !(el.matches('.photoref') && el.hidden);
      });
    }
    function set(el) { if (cur) cur.classList.remove('tvcur'); cur = el; if (el) el.classList.add('tvcur'); }
    function index(list) {
      var i = cur ? list.indexOf(cur) : -1; if (i >= 0) return i;
      var y = window.innerHeight * 0.2, best = 0, bd = Infinity;  /* nothing lit yet: the block nearest the reading line */
      list.forEach(function (el, k) { var d = Math.abs(el.getBoundingClientRect().top - y); if (d < bd) { bd = d; best = k; } });
      return best;
    }
    function goTo(list, i) {
      i = Math.max(0, Math.min(list.length - 1, i)); var el = list[i]; if (!el) return; set(el);
      var top = el.getBoundingClientRect().top + window.scrollY - (el.matches('.stage, h2') ? 24 : window.innerHeight * 0.2);
      window.scrollTo({ top: Math.max(0, top), behavior: 'smooth' });
    }
    function step(d) { var list = blocks(); if (!list.length) return; goTo(list, (cur ? index(list) : index(list) - Math.max(0, d)) + d); }
    function day(d) {
      var list = blocks(), stages = list.filter(function (el) { return el.matches('.stage'); }); if (!stages.length) return;
      var here = cur ? cur.closest('.stage') : null, i = here ? stages.indexOf(here) : -1;
      if (i < 0) { var y = window.innerHeight * 0.2, bd = Infinity; stages.forEach(function (st, k) { var dd = st.getBoundingClientRect().top - y; if (dd <= 1 && -dd < bd) { bd = -dd; i = k; } }); }
